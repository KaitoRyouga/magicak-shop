...
class GodaddyService
{
    ...
    const API_URL_SANDBOX = 'https://api.ote-godaddy.com/';
    ...
    public function __construct(
        ...
        $this->httpHeaders = [
            ...
            'Authorization' => 'sso-key ' . config('godaddy.api_key_sandbox') . ':' . config('godaddy.secret_key_sandbox')
        ];
    }
    ...
    public function checkAvailableListDomain(array $data): array
    {
        try {
            ...
            $response = $this->client->post(self::API_URL_SANDBOX . self::API_VERSION . '/domains/available', [
                'params' => [
                    'checkType' => 'FAST',
                    'forTransfer' => 'false',
                ],
            ...
            if ($response->getStatusCode() == 200) {
                ...
                if (!empty($output)) {
                    foreach ($output->domains as $key => $value) {
                        ...
                        // TODO: fix later
                        $result[$key]['is_available'] = true;
                        $result[$key]['price'] = $value->available ? round($value->price / 16856) : 1;
    ...
    {
        try {
            $url = self::API_URL_SANDBOX . self::API_VERSION . '/domains/' . $data['domain_name'] . '/records/' . $data['record_type'] . '/' . $data['subdomain'];
    ...
    public function createDomain(array $data)
    {
        try {
            ...
            $response = $this->client->post(self::API_URL_SANDBOX . self::API_VERSION . '/domains/purchase', [
                'params' => [
                    'X-Shopper-Id' => config('godaddy.shopper_id_sandbox'),
                ],
                'json' => $data,
    ...
    {
        $url = self::API_URL_SANDBOX . self::API_VERSION . '/domains/' . $data['domain_name'];
        $response = $this->client->get($url, [
            'params' => [
                'X-Shopper-Id' => config('godaddy.shopper_id_sandbox'),
            ],
    ...
    {
        $url = self::API_URL_SANDBOX . self::API_VERSION . '/domains/' . $domain_name . '/records';
    ...
    {
        $url = self::API_URL_SANDBOX . self::API_VERSION . '/domains/' . $domain_name . '/records/A';
        $response = $this->client->get($url, [
            'params' => [
                'X-Shopper-Id' => config('godaddy.shopper_id_sandbox'),
            ],
    ...
    {
        $url = self::API_URL_SANDBOX . self::API_VERSION . '/domains/' . $domain_name . '/records/A/' . $record_name;
     ...
     * @return mixed
     */
    public function updateRecordDNS(string $domain_name, string $record_name, array $data, string $record_type = "A")
    {
        $url = self::API_URL_SANDBOX . self::API_VERSION . '/domains/' . $domain_name . '/records/' . $record_type . '/' . $record_name;
